...
import { useState, useRef, useEffect, useCallback, useMemo, JSX, forwardRef, useImperativeHandle } from "react";
...
const getCheckState = (
  ...
  // Get the fresh item from the map
  ...
  if (!originalItem.children || originalItem.children.length === 0) {
    return originalItem.checked === true ? "checked" : "unchecked";
  }
...
export const updateParentStates = (items: TreeViewItem[]): TreeViewItem[] => {
  return items.map(item => {
    if (item.children) {
      ...
      // Then calculate this parent's state based on children
      const allChildrenChecked = updatedChildren.every(child => child.checked === true);
...
// Helper function to update item checked state with proper parent state propagation
export const updateItemCheckedState = (items: TreeViewItem[], targetId: string, checked: boolean): TreeViewItem[] => {
  const updateItems = (items: TreeViewItem[]): TreeViewItem[] => {
    return items.map(item => {
      if (item.id === targetId) {
        // Update the target item and all its children
        const updateChildren = (children?: TreeViewItem[]): TreeViewItem[] | undefined => {
  ...
  // First update the target item and its children, then update all parent states
...
function TreeItem({
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    // Early return if selection is disabled
    if (disableSelection) {
      // Still allow folder expansion on click
      if (item.children) {
  ...
  const handleAccessClick = (e: React.MouseEvent) => {
    ...
    if (onAccessChange) {
      ...
      // Apply change to this item and all its descendants
      const processItem = (item: TreeViewItem) => {
    ...
    <div>
      <div
        ...
        className={`select-none cursor-pointer ${
          isSelected && !disableSelection ? `bg-secondary ${selectionStyle}` : "text-foreground"
        } px-1`}
            ...
            <div className="flex items-center gap-2 flex-1 group">
              ...
              {showAccessRights && (
                <Input
                  ...
                  checked={getCheckState(item, itemMap) === "checked"}
                  ref={(checkbox: HTMLInputElement | null) => {
                    if (checkbox) {
                      checkbox.indeterminate = getCheckState(item, itemMap) === "indeterminate";
                    }
                  }}
                  onChange={(e) => handleAccessClick(e as any)}
                  onClick={(e) => e.stopPropagation()}
                  className="w-4 h-4 cursor-pointer hover:opacity-80"
                />
              )}
              {showIcons && renderIcon()}
              <span className={`flex-1 text-sm ${readonly ? "opacity-60" : ""}`}>{item.name}</span>
              ...
              <CollapsibleContent forceMount asChild>
                ...
                >
                  {item.children?.map((child) => (
                    <TreeItem
                      key={`${child.id}-${child.checked ? 'checked' : 'unchecked'}`}
                      item={child}
                      depth={depth + 1}
...
export default forwardRef<TreeViewRef, TreeViewProps>(function TreeView({
  className,
  checkboxLabels = {
    check: "Check",
    uncheck: "Uncheck",
  ...
  // Create a map of all items by ID - update when data changes
  ...
  useEffect(() => {
    // Don't handle click away if selection is disabled
  ...
  // Get checked items
  const getCheckedItems = useCallback((): TreeViewItem[] => {
    const items: TreeViewItem[] = [];
    const processItem = (item: TreeViewItem) => {
  ...
  // Expose getCheckedItems function to parent components
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    // Don't allow drag selection if selection is disabled
    if (disableSelection) return;
  ...
  const handleMouseMove = useCallback(
    ...
    [isDragging, dragStart, selectedIds, dragStartPosition, disableSelection]
  );
    ...
    <div className="flex gap-4 pt-1">
      ...
      >
        <AnimatePresence mode="wait">
          {selectedIds.size > 0 && showSelected && !disableSelection ? (
            <motion.div
              key="selection"
        ...
        >
          ...
          {(() => {
            // If hideRootFolder is true and there's only one root item with children, render its children directly
            if (hideRootFolder && filteredData.length === 1 && filteredData[0].children) {
              return filteredData[0].children.map((child) => (
                <TreeItem
                  key={`${child.id}-${child.checked ? 'checked' : 'unchecked'}`}
                  item={child}
                  selectedIds={selectedIds}
            ...
            // Default rendering - show all root items
            return filteredData.map((item) => (
              <TreeItem
                key={`${item.id}-${item.checked ? 'checked' : 'unchecked'}`}
                item={item}
                selectedIds={selectedIds}
                lastSelectedId={lastSelectedId}
                onSelect={setSelectedIds}
